...
from datasets.datasets import CategoriesSampler, build_dataset
...
def get_args():
    parser = argparse.ArgumentParser(
        "TransVLAD fine-tuning and evaluation script for image classification",
        add_help=False,
    )
    ...
    parser.add_argument("--pooling", default="nextvlad", type=str)
    ...
    parser.add_argument(
        "--finetune",
        default="output/zlm_pretrain/FC100-1599.pth",
    ...
    parser.add_argument(
        "--root_path",
        default="datasets/datasets/mini-imagenet",
        type=str,
        help="dataset path",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--nb_classes", default=64, type=int, help="number of the classification types"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        default="output/finetune_output/test",
    ...
    parser.add_argument("--meta_val", action="store_true", default=False)
    parser.add_argument("--meta_test", action="store_true", default=False)
...
def main(args, ds_init):
    ...
    dataset_train, args.nb_classes = build_dataset(
        is_train=True, dataset_name=args.dataset_name, split="train", args=args
    )

    if args.val:
        dataset_val, _ = build_dataset(
            is_train=False, dataset_name=args.dataset_name, split="val", args=args
        )
